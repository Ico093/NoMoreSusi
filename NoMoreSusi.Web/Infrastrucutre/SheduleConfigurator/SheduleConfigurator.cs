...
namespace NoMoreSusi.Web.Infrastrucutre.SheduleConfigurator
{
    public static class SheduleConfigurator
    {
                    ...
                    if (!_data.Shedules.All()
                        .Any(s =>
                                s.RoomId == lecture.RoomId && s.Day == lecture.Day &&
                                    ((s.Hour <= lecture.Hour &&
                                    s.Hour + s.Lecture.Length > lecture.Hour) ||
                                    (s.Hour < lecture.Hour + lecture.Length &&
                                    s.Hour + s.Lecture.Length >= lecture.Hour + lecture.Length))) &&
                                lecture.Room.PeopleCount >= lecture.Students.Count)
                    {
        ...
        private static bool FindRoomAndHour(Lecture lecture, DayOfWeek day)
        {
            ...
            var shedules = _data.Shedules.All().Where(s => s.RoomId == lecture.RoomId && s.Day == day).ToList();

            for (int currentHour = lecture.Hour; currentHour >= min_hour; currentHour--)
            {
                if (!shedules.Any(s => (s.Hour <= currentHour &&
                                    s.Hour + s.Lecture.Length > currentHour) ||
                                    (s.Hour < currentHour + lecture.Length &&
                                    s.Hour + s.Lecture.Length >= currentHour + lecture.Length)) && lecture.Room.PeopleCount >= lecture.Students.Count)
                {
        ...
        private static bool FindRoom(Lecture lecture, int hour, DayOfWeek day)
        {
            ...
            var rooms = _data.Rooms.All().Where(r => r.Id != lecture.Room.Id && r.PeopleCount >= lecture.Students.Count).ToList();
        ...
        private static bool FindExact(Lecture lecture, int hour, Room room, DayOfWeek day)
        {
            if (!_data.Shedules.All()
                    .Any(s => s.Day == day &&
                        ((s.Hour <= hour &&
                        s.Hour + s.Lecture.Length > hour) ||
                        (s.Hour < hour + lecture.Length &&
                        s.Hour + s.Lecture.Length >= hour + lecture.Length)) &&
                        s.RoomId == room.Id))
            {
